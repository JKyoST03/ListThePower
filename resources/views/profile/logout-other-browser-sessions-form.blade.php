<x-action-section>
    ...
    <x-slot name="description">
        {{ __('Administra las sesiones que tengas en otros navegadores, y cierra sesión en estás si no estás seguro de si te has logueado en una de estás sesiones.') }}
    </x-slot>
    ...
    <x-slot name="content">
                            ...
                            <div>
                                <div class="text-xs text-gray-100">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('Este disp.') }}</span>
                                    @else
                                        {{ __('Último disp') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="flex items-center mt-5">
            <x-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('Cierra sesión en los navegadores') }}
            </x-button>
        ...
        <x-dialog-modal wire:model.live="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('Por favor, introduce tu contraseña para confirmar que deseas cerrar sesión en tus otros navegadores y dispositivos.') }}
